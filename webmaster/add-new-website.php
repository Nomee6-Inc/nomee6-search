...
if (!isset($_SESSION['username'])) {
    ...
    if(isset($_POST['adddomainsubmit'])) {
        $getdomain = $_POST['domaininput'];
        $getdomain = str_replace("https://", "", $getdomain);
        $getdomain = str_replace("http://", "", $getdomain);
        $getdomain = str_replace("/", "", $getdomain);
        $getdomain = str_replace(":", "", $getdomain);

        $sql = "SELECT * FROM websites WHERE domain = '$getdomain'";
        $result = mysqli_query($conn, $sql);

        $getrandomdnsverifytoken = openssl_random_pseudo_bytes(20);
        $getrandomdnsverifytoken = bin2hex($getrandomdnsverifytoken);

        if (!$result->num_rows > 0) {
            $sql0 = "INSERT INTO websites (domain, author, dnsverifystatus, dnsverifyrecord)
            VALUES ('$getdomain', '$getuseremail', 'no', '$getrandomdnsverifytoken')";
            $result0 = mysqli_query($conn, $sql0);
            if($result0) {
                header("Location: dns-verify?domain=$getdomain");
            } else {
            ...
            echo "<div class=\"alert alert-warning\">
                    <h4 class=\"lang alert-title\" key=\"domainalreadyregisteredwebmaster\" style=\"text-align:center\">The domain name is already registered in our system.</h4>
                  </div>";
        }
    }
}
...
<html lang="tr">
  <head>
    ...
    <title>Add New Website | NOMEE6 Search</title>
    <!-- CSS files -->
        ...
        <div class="container-xl">
          <div class="page-header d-print-none">
            <div class="row align-items-center">
              <div class="col">
                <h2 class="lang page-title" key="addwebsite">
                    Add Website
                </h2>
              </div>
            </div>
            ...
            <div class="col-12">
                <form action="" method="POST">
                  <div class="card">
                    <div class="card-body">
                      <div class="mb-3">
                        <div class="lang form-label" key="domain">Domain</div>
                        <input name="domaininput" class="form-control" autocomplete="off" maxlength="500" required/>
                      </div>
                      <div class="mt-2">
                        <button type="submit" name="adddomainsubmit" class="btn btn-primary w-100">
                          <qwe class="lang" key="next">Next</qwe> &nbsp;
